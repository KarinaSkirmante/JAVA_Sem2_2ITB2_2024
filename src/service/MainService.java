...
public class MainService {
	...
	public static void main(String[] args) {
		...
		Professor pr2 = new Professor("Karina", "Šķirmante", Degree.mg, "121200-35798");
		allPersons.add(pr1);
		allPersons.add(pr2);
		...
		try {
			...
			System.out.println(temptSt.getName() + " " + temptSt.getSurname() + "-> vidējā atzīme ir "
					+ calculateAVGgradeByStudent(temptSt));

			System.out.println(c2.getTitle() + " -> vidēja atzīme ir " + calculateAVGgradeInCourse(c2));

			System.out.println(pr2.getName() + " " + pr2.getSurname() + " pasniedz "
					+ calculateHowManyCoursesByProfessor(pr2) + " kursus");
	...
	public static int calculateHowManyCoursesByProfessor(Professor professor) throws Exception {
		if (professor == null)
			throw new Exception("Problems with input arguments");
		...
		for (Person tempSt : allPersons) {
			if (tempSt instanceof Student) {
				if (tempSt.getPersonCode().equals(personCode)) {
					throw new Exception(
							tempSt.getName() + " " + tempSt.getSurname() + " is already registered in the system");
				}
			}
		}
	...
	public static Student retrieveStudentByPersonCode(String personCode) throws Exception {
		...
		// 2. ejot cauri foreach cikla visiem studentiem, atrast konkrēto pēc personas
		// koda
		for (Person tempSt : allPersons) {
			if (tempSt instanceof Student) {
				if (tempSt.getPersonCode().equals(personCode)) {
